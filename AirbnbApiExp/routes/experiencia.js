...
router.put('/:id_experiencia', function(req, res, next) {
  ...
  const{checkin,checkout,numero_personas,fecha,id_usuario,id_departamento}=req.body;
    pool.getConnection(function(err, connection) {
      connection.query('Update usuario set checkin="'+checkin+'", checkout="'+checkout+'", numero_personas="'+numero_personas+'", fecha=NOW(), id_usuario="'+id_usuario+'", id_departamento="'+id_departamento+'" where id="'+id_experiencia+'"' , function (error, results, fields) {
          connection.release();
          if (error) throw error;
          res.json(results);  //We return the respone with all the information needed
      });
